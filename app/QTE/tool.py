import time
from core import ESC,esc,esqt
# Tool class defination;
yu=esqt.YU
class ToolDiv(esqt.Tab):
    def __init__(self, parent, **argkw) -> None:
        super().__init__(parent, **argkw)
        self.base_div=esqt.Box(None)
        self.addTab(self.base_div,'Base')
        return

    def buildBaseTab(self):
        self.base_div.delChildren()
        TP=self.base_div
        menu_aro=AroMenu(TP,'menu_aro',item=ARO_INDEX,label='Aroes',
            size=(8*yu,4*yu))
        menu_acp=AcpMenu(TP,'menu_acp',item=ACP_INDEX,label='Acps',
