...
class SimItemDiv(esui.TreeItemDiv):
    ...
    def updateItem(self):
        ...
        if self.plabel=='Model':
            if (ESC.SIM_NAME,self.label) in ESC.PERFERENCE['MODEL_ENABLE']:
    ...
    def drawBadges(self):
        for ctrl in self.getChildren():
            ...
            if pctrl.label=='Model':
                if (ESC.SIM_NAME,ctrl.label) in ESC.PERFERENCE['MODEL_ENABLE']:
...
class MdlTreeDiv(esui.TreeDiv):
    ...
    def onDClkItem(self,e):
        mt=self.getModelTuple(e.EventObject)
        esui.toggleWorkspace(target='ACP')
        esui.ACP_PLC.drawAcp(mdl_tuple=mt)
        e.Skip()
        return
    ...
    def drawBadges(self):
        for ctrl in self.getChildren():
            ...
            if ctrl.depth==1:
                if (mod,ctrl.label) in ESC.PERFERENCE['MODEL_ENABLE']:
                    ctrl.dict_badge['enable mdl']=esui.Div(ctrl,style=dict(self.stl_en))
                else:
                    ctrl.dict_badge['enable mdl']=esui.Div(ctrl,style=dict(self.stl_dis))
        return
