class Quaternion(object):
    def __init__(self, s,**argkw):
        ''' Para s: num;

            Para argkw:

            * Kw vector: list of vec3;

            * Kw x/y/z: num.'''
        self.s = s
        vector=argkw.get('vector',None)
        if vector is None:
            self.x =argkw.get('x')
            self.y =argkw.get('y')
            self.z =argkw.get('z')
    ...
    def __add__(self, quater):
        ...
        for i in range(4):
            q[i] += quater.all[i]
        return Quaternion(q[0],x=q[1],y=q[2],z=q[3])
    ...
    def __mul__(self, quater):
        ...
        return Quaternion(s, x=x, y=y, z=z)
