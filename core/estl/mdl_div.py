...
class AcpCanvas(wx.ScrolledCanvas):
    ...
    def rmAcpNodes(self):
        ...
        acps=[node.acp for node in self.list_active]
        rm_acp=ESC.clipAcps(acps,self.Parent.mdl)
    ...
    def onRls(self,e):
        ...
        # Rect selecting;
        for node in self.Children:
            cx=node.Position[0] in esc.Itv(cpos[0],self.spos[0])
            cy=node.Position[1] in esc.Itv(cpos[1],self.spos[1])
            if cx and cy:
                self.list_active.append(node)
                node._flag_active=True
            elif node._flag_active:
                node._flag_active=False
            node.Refresh()
        self.drawLink()
        return
...
class AcpPortDiv(esui.Div):
    def __init__(self, parent, **argkw):
        super().__init__(parent, **argkw)
        w=parent.Size.x-2*yu
        self.updateStyle(style={'s':(w,3*yu),'bgc':esui.COLOR_LBACK})
        self.port:esc.AcpPort=argkw['port']
        self.txt_pid=esui.DivText(self,
            label=self.port.io[0].upper()+'-'+str(self.port.pid)+':',
            style={'p':(0,0),'s':(3*yu,3*yu),'align':'left'})
        flagin=self.GrandParent.acp.fix_in.value and self.port.io=='in'
        flagout= self.GrandParent.acp.fix_out.value and self.port.io=='out'
        flag=flagout or flagin
        self.input_name=esui.InputText(self,
            hint=self.port.name,readonly=flag,
            style={'p':(4*yu,0),'s':(w-12*yu,3*yu)})
        self.btn_con=esui.DivBtn(self,label='√',enable=not flag,
            style={'p':(w-7*yu,0),'s':(3*yu,3*yu)})
        self.btn_del=esui.DivBtn(self,label='×',enable=not flag,
            style={'p':(w-3*yu,0),'s':(3*yu,3*yu)})
        if not flag:self.btn_con.Bind(esui.EBIND_LEFT_CLK,self.onClkCon)
        if not flag:self.btn_del.Bind(esui.EBIND_LEFT_CLK,self.onClkDel)
        return

    def onClkCon(self,e):
        self.GrandParent.acp.setPort(pid=self.port.pid,name=self.input_name.getValue())
        self.GrandParent.showDetail()
        return

    def onClkDel(self,e):
        self.GrandParent.acp.delPort(self.port.pid)
        self.GrandParent.showDetail()
        return
    pass
class DetailDialog(esui.EsDialog):
    ...
    def showDetail(self):
        ...
        for name,attr in acpattr.items():
            if not isinstance(attr,esc.AcpAttr):continue
        ...
        esui.StaticText(PP,(yu,0),(8*yu,4*yu),'IO Ports:',align='left')
        btn_out=esui.Btn(PP,(PP.Size.x-8*yu,yu/2),(3*yu,3*yu),'+Out')
        btn_in=esui.Btn(PP,(PP.Size.x-4*yu,yu/2),(3*yu,3*yu),'+In')
        ...
        for pid,port in acp.port.value.items():
            AcpPortDiv(PP,port=port,style={'p':(yu,i*4*yu)})
            i+=1
        DP.updateMaxSize()
        PP.updateMaxSize()
        return
    ...
    def onClkAddInBtn(self,e):
        ports=self.acp.getPort()
        if len(ports)==0:maxid=1
        else:maxid=max([port.pid for port in ports])+1
    ...
    def onClkAddOutBtn(self,e):
        ...
        self.acp.addPort(pid=maxid,io='out',name='New')
        self.showDetail()
        return
